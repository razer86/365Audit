...
.DESCRIPTION
    Reads the customer list from UnattendedCustomers.json (one entry per customer),
...
.PARAMETER Customers
    ...
    all entries in UnattendedCustomers.json are processed.

.PARAMETER Modules
    Global module override — applies to every customer this run.
    When omitted, each customer uses the Modules array from the JSON file.
...
.EXAMPLE
    ...
    Processes all customers in UnattendedCustomers.json using HUDU_API_KEY from env.
...
# ── Load customer list from JSON ───────────────────────────────────────────────
$customerListPath = Join-Path $PSScriptRoot 'UnattendedCustomers.json'
if (-not (Test-Path $customerListPath)) {
    Write-Error ("Customer list not found: $customerListPath`n" +
        "Copy UnattendedCustomers.json.example to UnattendedCustomers.json " +
        "and add your customers.") -ErrorAction Stop
}

$parsed = Get-Content $customerListPath -Raw | ConvertFrom-Json
$customerList = if ($parsed.PSObject.Properties['customers']) { $parsed.customers } else { $parsed }

if ($customerList.Count -eq 0) {
    Write-Error "No customers defined in $customerListPath." -ErrorAction Stop
}
...
if ($Customers) {
    ...
    if ($customerList.Count -eq 0) {
        Write-Error "None of the specified customers matched entries in $customerListPath." -ErrorAction Stop
    }
}
# ─────────────────────────────────────────────────────────────────────────────
