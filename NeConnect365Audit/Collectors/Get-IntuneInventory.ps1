function Get-IntuneInventory {
    <#
    .SYNOPSIS
        Collects Intune / Endpoint Manager inventory data and exports to CSV files.

    .DESCRIPTION
        Queries Microsoft Graph for Intune device management data including:
        ...
        Graph connection must already be established by the orchestrator.
    ...
    # ── Import Graph sub-modules for Intune queries ───────────────────────
    Import-GraphModule -ModuleName 'Microsoft.Graph.DeviceManagement'
    Import-GraphModule -ModuleName 'Microsoft.Graph.Devices.CorporateManagement'
    Import-GraphModule -ModuleName 'Microsoft.Graph.DeviceManagement.Enrollment'

    Write-Host "`nStarting Intune Inventory for $($ctx.OrgName)..." -ForegroundColor Cyan

    $step       = 0
    $totalSteps = 10
    $activity   = "Intune Inventory — $($ctx.OrgName)"
    ...
    # ── Track summary counts for return ───────────────────────────────────
    $_deviceCount          = 0
    $_compliancePolicyCount = 0
    $_configProfileCount   = 0
    $_appCount             = 0
    $_autopilotCount       = 0
    $_enrollmentConfigCount = 0
    $_updateRingCount      = 0
    $_mamPolicyCount       = 0
    ...
    if (-not $_hasIntune) {
        Write-Warning "No Intune-capable licence found for $($ctx.OrgName). Skipping Intune data collection."
    ...
    try {
        ...
        $_deviceCount = $_allDevices.Count
    ...
    try {
        ...
        $_compliancePolicyCount = $_policies.Count
    ...
    try {
        ...
        $_configProfileCount = $_profileRows.Count
    ...
    try {
        ...
        $_autopilotCount = $_autopilot.Count
    ...
    try {
        ...
        $_enrollmentConfigCount = $_restrictions.Count
    ...
    try {
        ...
        $_updateRingCount = $_updateRingObjs.Count
    ...
    try {
        ...
        $_mamPolicyCount = $_mamPolicies.Count
    ...
    Write-Progress -Id 1 -Activity $activity -Completed
    Write-Host "`nIntune Inventory complete. Results saved to: $outputDir" -ForegroundColor Green

    return @{
        DeviceCount          = $_deviceCount
        CompliancePolicyCount = $_compliancePolicyCount
        ConfigProfileCount   = $_configProfileCount
        AppCount             = $_appCount
        AutopilotCount       = $_autopilotCount
        EnrollmentConfigCount = $_enrollmentConfigCount
        UpdateRingCount      = $_updateRingCount
        MamPolicyCount       = $_mamPolicyCount
        HasIntune            = $true
    }
}
