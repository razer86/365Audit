function Get-SharePointInventory {
    <#
    .SYNOPSIS
        Collects SharePoint Online and OneDrive inventory data and exports to CSV files.
    ...
    .DESCRIPTION
        ...
        - SharePoint groups and members per site (parallel per-site collection)
        ...
        Graph must already be connected by the orchestrator. PnP.PowerShell connections
        are established internally using credentials from the audit context.
    ...
    # ── Load Graph sub-modules required by this collector ─────────────────
    Import-GraphModule -ModuleName 'Microsoft.Graph.Users'          # Get-MgUser (unlicensed OneDrive detection)

    # ── Resolve credentials from audit context ────────────────────────────
    $_spAppId        = $ctx.AppId
    $_spCertFilePath = $ctx.CertFilePath
    $_spCertPassword = $ctx.CertPassword
    $_useAppAuth     = $_spAppId -and $_spCertFilePath

    # SecureString cannot be serialized across parallel runspace boundaries — extract
    # plain text here and reconstruct a new SecureString inside each runspace.
    # PtrToStringBSTR is used instead of PtrToStringAuto for Linux compatibility.
    ...
    Write-Host "`nStarting SharePoint Online Inventory for $($ctx.OrgName)..." -ForegroundColor Cyan

    $step       = 0
    $totalSteps = 8
    $activity   = "SharePoint Inventory — $($ctx.OrgName)"

    # ── Tracking counters for summary ─────────────────────────────────────
    $_siteCount              = 0
    $_oneDriveCount          = 0
    $_unlicensedOneDriveCount = 0
    $_siteOverrideCount      = 0
    ...
    } | Export-Csv -Path (Join-Path $outputDir 'SharePoint_TenantStorage.csv') -NoTypeInformation -Encoding UTF8
    ...
    $siteReport | Export-Csv -Path (Join-Path $outputDir 'SharePoint_Sites.csv') -NoTypeInformation -Encoding UTF8
    ...
    $groupReport      | Export-Csv -Path (Join-Path $outputDir 'SharePoint_SPGroups.csv')        -NoTypeInformation -Encoding UTF8
    $permissionReport | Export-Csv -Path (Join-Path $outputDir 'SharePoint_SitePermissions.csv')  -NoTypeInformation -Encoding UTF8
    ...
    $tenantConfig |
        ...
        Export-Csv -Path (Join-Path $outputDir 'SharePoint_ExternalSharing_Tenant.csv') -NoTypeInformation -Encoding UTF8
    ...
    $_oneDriveCount = @($oneDriveReport).Count
    $oneDriveReport | Export-Csv -Path (Join-Path $outputDir 'SharePoint_OneDriveUsage.csv') -NoTypeInformation -Encoding UTF8
    ...
    $_siteOverrideCount = @($siteOverrides).Count
    ...
    $siteSharingReport | Export-Csv -Path (Join-Path $outputDir 'SharePoint_ExternalSharing_SiteOverrides.csv') -NoTypeInformation -Encoding UTF8
    ...
    } | Export-Csv -Path (Join-Path $outputDir 'SharePoint_AccessControlPolicies.csv') -NoTypeInformation -Encoding UTF8
    ...
    $_unlicensedOneDriveCount = @($unlicensedOneDrives).Count
    $unlicensedOneDrives | Export-Csv -Path (Join-Path $outputDir 'SharePoint_OneDrive_Unlicensed.csv') -NoTypeInformation -Encoding UTF8
    ...
    Write-Host "SharePoint Online Inventory complete. Results saved to: $outputDir" -ForegroundColor Green

    return @{
        SiteCount              = $_siteCount
        OneDriveCount          = $_oneDriveCount
        UnlicensedOneDriveCount = $_unlicensedOneDriveCount
        SiteOverrideCount      = $_siteOverrideCount
    }
}
