function Register-AuditApp {
    <#
    .SYNOPSIS
        Registers or maintains the 365Audit app registration in a customer tenant.

    .DESCRIPTION
        First run  : Creates the audit app registration with all required permissions
                     (Graph, Exchange, SharePoint), grants admin consent, generates a
                     self-signed certificate (.pfx), and returns credentials for storage.
        ...
        When -HuduCompanyId or -HuduCompanyName is supplied, credentials are automatically
        pushed to the matching Hudu company asset.
    ...
    .PARAMETER HuduCompanyId
        ...
        to the matching Hudu company's audit toolkit asset automatically.
    ...
    .PARAMETER HuduBaseUrl
        Hudu instance base URL. Falls back to 'https://neconnect.huducloud.com'.

    .PARAMETER HuduApiKey
        Hudu API key.

    .OUTPUTS
        PSCustomObject with App, TenantId, OrgName, CertificateThumbprint properties.
    ...
    .EXAMPLE
        Register-AuditApp -Force
        Force certificate renewal even when the current certificate is healthy.

    .EXAMPLE
        Register-AuditApp -HuduCompanyId 'contoso' -HuduApiKey $key
        Setup with automatic Hudu credential push.
    #>
    ...
    # Output directory for generated .pfx files (temp directory)
    $script:CertOutputDir = [System.IO.Path]::GetTempPath()
    ...
    function Set-ExchangeAdminRole {
        ...
        try {
            ...
            if ($_.Exception.Message -match 'already exist') {
                ...
                Write-Warning "ACTION REQUIRED: In Entra ID, assign the 'Exchange Administrator' role to the '$AppName' service principal manually."
            }
        }
    }
    ...
    function Set-GlobalReaderRole {
        ...
        try {
            ...
            if ($_.Exception.Message -match 'already exist') {
                ...
                Write-Warning "ACTION REQUIRED: In Entra ID, assign the 'Global Reader' role to the '$AppName' service principal manually. This role is required by Maester."
            }
        }
    }
    ...
    function Write-CredentialSummary {
        ...
        Write-Host "  Invoke-TenantAudit -AppId '$AppId' -TenantId '$TenantId' -CertBase64 '<paste base64>' -CertPassword (Read-Host -AsSecureString 'Cert Password')" -ForegroundColor Cyan
    ...
    function Push-HuduAuditAsset {
        ...
        if (-not $huduUrl -or -not $huduKey) {
            ...
            Write-Host '  Pass -HuduApiKey and -HuduBaseUrl to enable Hudu integration.' -ForegroundColor DarkGray
            return
        }
        ...
        $launchCmd = "<p><strong>Without Hudu API key:</strong><br>" +
                     "Invoke-TenantAudit -AppId '$AppId' -TenantId '$TenantId' -CertPassword (Read-Host -AsSecureString 'Cert Password')</p>" +
                     "<p><strong>With Hudu API key:</strong><br>" +
                     "Invoke-TenantAudit -HuduCompanyId '$companySlug'</p>"
    ...
    # Main execution flow
    # ============================================================
    # Track output result for returning at the end
    $_resultApp       = $null
    $_resultTenantId  = $null
    $_resultOrgName   = $null
    $_resultThumbprint = $null

    try {
        Write-Status 'Register-AuditApp starting...' -Type Info
        ...
        if ($AppId -and $TenantId) {
            ...
            $_resultOrgName  = $permResult.OrgName
            ...
            if (-not $certStatus.HasActive) {
                ...
                $_resultThumbprint = $newCert.PfxPath
            }
    ...
    if ($_resultApp) {
        [PSCustomObject]@{
            App                    = $_resultApp
            TenantId               = $_resultTenantId
            OrgName                = $_resultOrgName
            CertificateThumbprint  = $_resultThumbprint
        }
    }
}
