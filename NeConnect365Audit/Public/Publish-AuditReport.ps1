function Publish-AuditReport {
    <#
    .SYNOPSIS
        Publishes a completed 365Audit report to a Hudu 'Monthly Audit Report' asset.

    .DESCRIPTION
        After a successful audit run this function:
          1. Reads ActionItems.json and AuditMetrics.json written by New-AuditSummary
    ...
    .PARAMETER ReportLayoutId
        Asset layout ID for the 'Monthly Audit Report' layout in Hudu. Defaults to 68.

    .PARAMETER ReportAssetName
        Prefix for the monthly report asset name. Asset is created as
        "<ReportAssetName> - yyyy-MM". Defaults to 'M365 Monthly Audit'.
    ...
    param (
        ...
        [string]$ReportAssetName = 'M365 Monthly Audit',
    ...
    if (-not $huduBodyFile) {
        Write-Warning "Publish-AuditReport: M365_HuduReport.html not found in '$OutputPath' — skipping."
        return
    }

    if (-not $fullReportFile) {
        Write-Warning "Publish-AuditReport: M365_AuditSummary.html not found in '$OutputPath' — full report attachment will be skipped."
    }
    ...
    if (-not $companyId) {
        try {
            ...
            Write-Warning "Publish-AuditReport: Company lookup failed — $_"
            return
        }
    }

    if (-not $companyId) {
        Write-Warning "Publish-AuditReport: Company slug '$CompanySlug' not found in Hudu — skipping."
        return
    }
    ...
    if (-not $assetId) {
        ...
        try {
            ...
            Write-Warning "Publish-AuditReport: Asset creation failed — $_"
            return
        }
    }
    else {
    ...
    try {
        ...
        Write-Warning "Publish-AuditReport: Delta computation failed (non-fatal) — $_"
    }
    ...
    catch { Write-Warning "Publish-AuditReport: Field update failed — $_" }
    ...
    if ($fullReportFile) {
        ...
        catch { Write-Warning "Publish-AuditReport: Full HTML upload failed — $_" }
    }
    ...
    try {
        ...
        Write-Warning "Publish-AuditReport: Compression failed — $_"
        return
    }
    ...
    try {
        ...
        $_localZip.Dispose()
        if ($_localCount -eq 0) {
            Write-Warning "Publish-AuditReport: Archive appears empty — skipping upload."
        ...
        Write-Warning "Publish-AuditReport: Archive verification failed — $_ — skipping upload."
        Remove-Item $zipPath -ErrorAction SilentlyContinue
        return
    }
    ...
    catch { Write-Warning "Publish-AuditReport: Zip upload failed — $_" }
    ...
    if ($CleanupLocal -and $_uploadSucceeded) {
        ...
        try {
            ...
            if (-not $_zipUrl) {
                Write-Warning "Publish-AuditReport: Could not resolve uploaded zip URL — local folder kept."
            }
            else {
                ...
                try {
                    ...
                    if ($_verifyCount -gt 0) {
                        ...
                        Write-Warning "Publish-AuditReport: Uploaded archive appears empty — local folder kept."
                    }
                }
                finally {
            ...
            Write-Warning "Publish-AuditReport: Cleanup verification failed — local folder kept: $_"
        }
    }
}
